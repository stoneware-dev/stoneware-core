...
const USAGE = `kiln - a Bun-native, server-first web framework

Usage
  kiln dev     [--root <dir>] [--port <n>]   Start the dev server with hot reload
  kiln build   [--root <dir>]                Production build (server + island bundles)
  kiln start   [--root <dir>] [--port <n>]   Run the production server bundle
...
async function main(): Promise<void> {
  ...
  // The CLI flag wins over kiln.config.ts, which wins over the default.
  ...
  switch (args.command) {
    ...
    case "build": {
      ...
      console.log(`[kiln] build complete in ${elapsed}ms`);
      console.log(describeBuild(result, args.root));
      break;
    }

    case "start": {
      const bundle = resolve(args.root, ".kiln", "server.js");
      if (!(await Bun.file(bundle).exists())) {
        console.error(`[kiln] No production build found at ${bundle}. Run \`kiln build\` first.`);
        process.exit(1);
      }
    ...
    default:
      console.error(`[kiln] Unknown command: ${args.command}\n`);
      console.log(USAGE);
      process.exit(1);
  }
}
