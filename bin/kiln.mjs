#!/usr/bin/env node
/**
 * Launcher for the `kiln` CLI.
 *
 * The CLI itself is Bun-only and always will be: `kiln dev` and `kiln build` are
 * built on `Bun.serve` and `Bun.build`, which have no Node equivalent worth
 * shimming. But the *entry point* is plain Node JavaScript so that `npx kiln`
...
const INSTALL_HINT = `[kiln] Kiln runs on Bun, and Bun was not found on your PATH.

  Install it:  https://bun.sh/docs/installation
    macOS/Linux  curl -fsSL https://bun.sh/install | bash
    Windows      powershell -c "irm bun.sh/install.ps1|iex"

Then re-run this command. (\`npx create-kiln\` works without Bun; running the
