...
export const DOCS: DocPage[] = [
  {
    ...
    summary: "The specific problems Kiln exists for — and the ones it does not.",
    blocks: [
      {
        kind: "p",
        text: "The web ships a lot of JavaScript to sites that do not need it. Kiln makes HTML the default and JavaScript the exception.",
      },
      {
        kind: "figure",
      ...
      {
        kind: "p",
        text: "Kiln is not a general-purpose replacement for anything. It was built for one shape of project: content-heavy, SEO-sensitive sites with a handful of interactive widgets. Within that shape, these are the problems it takes seriously.",
      },
      ...
      {
        kind: "p",
        text: "In Kiln a page with no islands ships no JavaScript. Not a small runtime, not a hydration shim — zero bytes, no script tag at all. That is asserted by the test suite, not just intended.",
      },
      {
        kind: "figure",
  ...
  (@preact/signals-core — everything else is Bun's own APIs)`,
      ...
      {
        kind: "p",
        text: "Kiln puts the boundary on the filesystem. Files under islands/ are bundler entry points; files under routes/ are never handed to the bundler at all. Server-only code does not stay server-only because a heuristic classified it correctly — it stays server-only because nothing can carry it across.",
      },
      {
        kind: "quote",
  ...
  Calculator    ──►  yes      islands/Calculator.tsx`,
      ...
      {
        kind: "p",
        text: "The header is the same markup on every page and will never change after it is painted. Kiln does not hydrate it, because there is nothing there to hydrate — it is a string the server produced, and it stays that way.",
      },
      ...
      {
        kind: "p",
        text: "A Kiln template is a plain function, called once per request, on the server. Nothing to memoize, no render loop to reason about, no rules-of-hooks lint plugin. Reactivity exists only inside islands, where it earns its keep — and an update writes to one text node rather than re-running a tree.",
      },
      ...
      },
      {
        ...
        items: [
          ...
          "A strict CSP ships by default and works, because Kiln never emits inline executable script. No nonces to plumb.",
      ...
      {
        kind: "p",
        text: "Reconciling a client tree against server markup produces a whole category of bug that only appears in production, usually as a warning nobody can reproduce. Kiln does not reconcile: it builds the island's tree and replaces the marked element outright. There is nothing to mismatch.",
      },
      {
        kind: "p",
      ...
      {
        kind: "p",
        text: "Kiln has one runtime dependency. Serving, bundling, escaping, CSRF tokens, routing, .env loading, and the test runner are all Bun's own APIs. That is a deliberate constraint, not a coincidence: if Bun ships it, Kiln does not add a package that reimplements it.",
      },
  ...
  everything else       ──►                       0 KB client JavaScript`,
      ...
      {
        kind: "p",
        text: "Worth stating plainly, because a framework that claims everything is useful for nothing. Kiln has nothing to say about databases, authentication, offline support, realtime collaboration, or large-scale client state. It is a rendering and routing layer with sensible security defaults, and that is the whole of it.",
      },
      {
        kind: "p",
      ...
      {
        kind: "p",
        text: "The fastest way to be disappointed by Kiln is to bring it to a problem it was not built for.",
      },
      {
        kind: "list",
        items: [
...

  {
    ...
    blocks: [
      ...
      },
      {
        ...
        language: "sh",
        label: "terminal",
        text: `bunx create-kiln my-site   # npx create-kiln my-site also works
cd my-site
bun install
bun run dev`,
      },
      { kind: "h2", text: "What you get" },
      {
        ...
        items: [
          ...
          "kiln.config.ts — port, CSP, CSRF settings.",
        ],
      },
      {
        kind: "p",
      ...
      { kind: "h2", text: "Your first page" },
      {
        ...
        text: `import type { PageProps } from "kiln";
...
export default function Home({ params }: PageProps) {
...

  {
    ...
    summary: "Every file create-kiln writes, what it is for, and what a build adds.",
    blocks: [
      {
        kind: "p",
      ...
      {
        kind: "figure",
        label: "bunx create-kiln my-site",
        text: `my-site/
│
...
│
├── kiln.config.ts             Port, CSP override, CSRF settings.
├── tsconfig.json              jsx: "react-jsx", jsxImportSource: "kiln"
├── package.json               scripts: dev / build / start
│
├── .env                       KILN_CSRF_SECRET, generated unique. Gitignored.
├── .env.example               Tracked template, no value.
└── .gitignore                 node_modules/ .kiln/ .env`,
      },
    ...
    └──► never passed to the bundler at all
      ...
      {
        kind: "p",
        text: "kiln build writes everything into .kiln/, which is gitignored. Deleting it is always safe.",
      },
      {
        kind: "figure",
        label: "kiln build",
...
│                           { "Counter": "/_kiln/Counter-jzp1gax8.js" }
│
├── static/                 Served under /_kiln/*, immutable (content-hashed).
...
└── entries/                Generated island entry points. Build input.`,
...

  {
    ...
    blocks: [
      {
        kind: "p",
        text: "Kiln is small enough to hold in your head. This page is the whole of it: one request pipeline, one render pass, and one hydration step.",
      },
      ...
      },
      {
        ...
        text: `  Request
     │
     ├─ /_kiln/*  ─────────────────►  built island chunk        (Bun.file)
     │
  ...
  HTML string`,
      ...
      },
      {
        ...
        data-kiln-island="LiveCounter"    <-- which island
        data-kiln-id="kiln-1">            <-- which instance
  fired <b>0</b> times
</button>

<script type="application/json" id="kiln-islands">
  [{"name":"LiveCounter","id":"kiln-1","props":{}}]
</script>

<script type="module" src="/_kiln/LiveCounter-6dhtkfqt.js"></script>`,
      },
      {
        kind: "p",
      ...
      { kind: "h2", text: "What hydration does to the DOM" },
      {
        ...
        text: `  BEFORE                              AFTER
  ──────                              ─────
  <button data-kiln-id="kiln-1">      <button data-kiln-id="kiln-1">
  ...
  count.value++
     ...
     │
     ├─ read #kiln-islands  ──► parsed as data, never evaluated
     │
     ...
     │      │
     │      └─ querySelector([data-kiln-id]).replaceWith(tree)
     │
...

  {
    ...
    blocks: [
      ...
      },
      {
        ...
        text: `import { signal } from "kiln/signals";
...
export default function Counter() {
  return (
    <button onClick={() => count.value++}>
      ...
      {
        kind: "quote",
        text: "An island must render exactly one HTML element at its root, because that element carries the marker. Kiln raises an explicit error rather than mis-hydrating.",
      },
...

  {
    ...
    blocks: [
      ...
      },
      {
        ...
        text: `import type { ActionContext } from "kiln";
...
<p>{raw("<em>ok</em>")}</p>  // the only way through`,
      ...
      {
        kind: "p",
        text: "A restrictive policy ships by default: script-src 'self', no unsafe-inline, no unsafe-eval. Kiln never emits inline executable script, so no nonce plumbing is needed to satisfy it.",
      },
      {
        kind: "quote",
    ...
    blocks: [
      {
        ...
        language: "sh",
        label: "terminal",
        text: `kiln dev     # dev server with hot reload
kiln build   # production build
kiln start   # run the production server bundle`,
      ...
      {
        kind: "p",
        text: "Bun reads .env natively, so Kiln has no dotenv dependency. create-kiln generates a .env with a unique KILN_CSRF_SECRET and gitignores it, leaving .env.example as the tracked template. A real environment variable beats .env.local, which beats .env.",
      },
    ],
  },
];
